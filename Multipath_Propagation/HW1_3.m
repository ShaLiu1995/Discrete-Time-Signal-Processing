...
figure;
plot(f, angle(H1_256));
title('arg(H1[k]) vs f, N = 64, NFFT = 256');
xlabel('Normalized Frequency')       
ylabel('arg(H1[k])');
axis([0 0.5*fs -pi pi]);
grid;
...
% c.6
NFFT = 256;
h2_256 = [h2 zeros(1, NFFT - 2*N)];
H2_256 = fft(h2_256, NFFT);
f = fs * (0 : NFFT - 1) / NFFT;

figure();
plot(f, abs(H2_256));
title('|H2[k]| vs f, 2*N = 128');
xlabel('Normalized Frequency')       
ylabel('|H2[k]|');
xlim([0 0.5]);
ylim([0 2.5]);
